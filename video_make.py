...
out = cv2.VideoWriter('pred_result_challenge_combine.avi',cv2.VideoWriter_fourcc('M', 'J', 'P', 'G') , 20, (96*2,96))
for n in range(5,353):
        # get a frame
    frame = cv2.imread("./pred_result_challenge_combine/%d.jpg" % n)
        # save a frame
    out.write(frame)
        # show a frame
    cv2.imshow("capture", frame)
